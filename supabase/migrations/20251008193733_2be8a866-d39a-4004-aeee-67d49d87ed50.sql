-- Atualizar função add_loyalty_points_on_completion para respeitar allow_points_accumulation em visitas também
    ...
    IF FOUND THEN
      ...
      ELSIF program.program_type = 'visitas' THEN
        IF program.qualifying_services IS NULL OR 
          ...
          -- Se allow_points_accumulation é false, verifica se ultrapassaria o limite
          IF NOT program.allow_points_accumulation THEN
            -- Buscar visitas atuais do cliente
            SELECT COALESCE(visits, 0) INTO current_visits
            FROM loyalty_balances
            WHERE business_id = NEW.business_id AND client_id = NEW.client_id;
            
            -- Limitar as visitas ao máximo necessário
            IF current_visits >= program.visits_required THEN
              visits_to_add := 0;
            END IF;
          END IF;
          
          -- Só adicionar se houver visitas a adicionar
          IF visits_to_add > 0 THEN
            INSERT INTO loyalty_balances (business_id, client_id, visits)
            VALUES (NEW.business_id, NEW.client_id, visits_to_add)
            ON CONFLICT (business_id, client_id) 
            DO UPDATE SET 
              visits = loyalty_balances.visits + visits_to_add,
              updated_at = NOW();
            ...
            INSERT INTO loyalty_transactions (
              ...
              NEW.business_id, NEW.client_id, 'ganho', visits_to_add,
